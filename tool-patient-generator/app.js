...
server.listen(port, () => console.log("Server at 7001"));
            ...
            .on('end', () => {
                ...
                for (var i = 0; i < dataRequest.total; i++) {
                    ...
                    const row = name + ',' +
                        ...
                        functions.parseDate(new Date(+(new Date()) - Math.floor(Math.random()*2700100000000)), 'dateType', false) + ',' + //DOB
...
function generateMessage(total, event, patIdAssigningFacility,patIdTypeIdentifier, assigningAuthority, patNHC, connection) {
    ...
    for (var i = 0; i < total; i++){
        ...
        const dateStartAppointment = functions.parseDate(new Date(+(new Date()) + 17001000), 'dateTime', false);
        const dateEndAppointment = functions.parseDate(new Date(+(new Date()) + (17001000 + durationMillis)), 'dateTime', false);
